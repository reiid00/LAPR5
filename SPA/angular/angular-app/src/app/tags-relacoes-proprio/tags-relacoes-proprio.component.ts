...
@Component({
  selector: 'app-tags-relacoes-proprio',
  templateUrl: './tags-relacoes-proprio.component.html',
  styleUrls: ['./tags-relacoes-proprio.component.css']
})
export class TagsRelacoesProprioComponent implements OnInit{
  ...
  async getTagsRelacoesProprio(){
    const relacoesFromService$ = this.relationService.getRelationFriends(this.currentUser.id).pipe(map(response=>response));
      
    const relacoes =await lastValueFrom(relacoesFromService$); 
    for(var i = 0; i< relacoes.length;i++)
    {
      for(var j = 0; j < relacoes[i].relationTypes.length;j++)
      {
        this.tags.push(relacoes[i].relationTypes[j]);
      }
    }
